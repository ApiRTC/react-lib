...
import {
	Conversation,
	ConversationCall,
	ConversationUnsubscribeToStream,
	PublishOptions,
	Stream,
	SubscribeOptions,
} from '@apirtc/apirtc';
	...
	return {
		...
		Conversation: jest.fn().mockImplementation((name: string, options?: any) => {
			const instance = {
				...
				isJoined: () => {
					return instance.joined;
				},
				isPublishedStream: (stream: Stream) => {
					return instance.publishedStreams.has(stream);
				},
				...
				unsubscribeToStream: (
					stream: Stream,
					options?: ConversationUnsubscribeToStream
				) => {},
...
describe('useConversationStreams', () => {
	...
	test(`conversation, joined, streams publication`, async () => {
		...
		const { result, rerender, waitForNextUpdate } = renderHook(
			...
			}) => useConversationStreams(props.conversation, props.streamsToPublish),
	...
	test(`streams publication with options change [{stream01, audioOnly}] to [{stream01, videoOnly}]`, async () => {
		...
		const { result, rerender, waitForNextUpdate } = renderHook(
			...
			}) => useConversationStreams(props.conversation, props.streamsToPublish),
			{
				initialProps: {
					conversation,
					streamsToPublish: [{ stream: stream01, options: audioOnly }],
				},
			}
		);
	...
	test(`streams publication [stream01, stream02] to [stream02, stream01]`, async () => {
		...
		const { result, rerender, waitForNextUpdate } = renderHook(
			...
			}) => useConversationStreams(props.conversation, props.streamsToPublish),
			{
				initialProps: {
					conversation,
					streamsToPublish: [{ stream: stream01 }, { stream: stream02 }],
				},
			}
		);
	...
	test(`streams publication [stream01], with publish error, with errorCallback`, (done) => {
		...
		const { result } = renderHook(
			(props: {
				...
				useConversationStreams(props.conversation, props.streamsToPublish, (error: any) => {
					done();
	...
	test(`streams publication [stream01, stream02] to [stream02, stream03], simulate publish error`, async () => {
		...
		rerender({
			conversation,
			streamsToPublish: [{ stream: stream02 }, { stream: stream03 }],
			errorCallback: (error: any) => {
